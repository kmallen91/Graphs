...
1. To create 100 users with an average of 10 friends each, how many times would you need to call `add_friendship()`? Why?
    - 500 times. 100 users with 10 friends each (100 * 10) = 1000, but that includes duplicates. Since friendships are bi-directional, we only need to call the function half as many times.
